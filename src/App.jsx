...
const InventorySystem = () => {
  ...
  // Filtros tabla de inventario
  const [filterInvLocation, setFilterInvLocation] = useState('all');
  const [filterInvBasket, setFilterInvBasket]     = useState('all');
  const [filterInvStatus, setFilterInvStatus]     = useState('all');
  const [filterInvType, setFilterInvType]         = useState('all');
  ...
  const filteredInventory = inventory.filter(t => {
    ...
    const matchInvLoc    = filterInvLocation === 'all' || t.location === filterInvLocation;
    const matchInvBasket = filterInvBasket === 'all' || (filterInvBasket === 'none' ? !t.basket_id : t.basket_id === filterInvBasket);
    const matchInvStatus = filterInvStatus === 'all' || t.status === filterInvStatus;
    const tubeType       = [t.diameter, t.material].filter(Boolean).join(' · ');
    const matchInvType   = filterInvType === 'all' || tubeType === filterInvType;
    return matchSearch && matchLoc && matchInvLoc && matchInvBasket && matchInvStatus && matchInvType;
  });

  const uniqueLocations = [...new Set(inventory.map(t => t.location).filter(Boolean))];
  const uniqueStatuses  = [...new Set(inventory.map(t => t.status).filter(Boolean))];
  const uniqueTypes     = [...new Set(inventory.map(t => [t.diameter, t.material].filter(Boolean).join(' · ')).filter(Boolean))];
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#1a2332', margin: 0 }}>
                Inventario ({filteredInventory.length} de {inventory.length})
              </h2>
              <SearchInput value={searchTerm} onChange={setSearchTerm} />
            </div>

            {/* Filtros dropdown */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px,1fr))', gap: '10px', marginBottom: '18px', padding: '14px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e5e7eb' }}>
              {/* Filtro ubicación */}
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#6b7280', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>📍 Ubicación</label>
                <select value={filterInvLocation} onChange={e => setFilterInvLocation(e.target.value)}
                  style={{ width: '100%', padding: '8px 10px', border: '2px solid #e5e7eb', borderRadius: '8px', fontSize: '13px', outline: 'none', cursor: 'pointer', background: 'white' }}>
              ...
              {/* Filtro canastilla */}
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#6b7280', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>🗂️ Canastilla</label>
                <select value={filterInvBasket} onChange={e => setFilterInvBasket(e.target.value)}
                  style={{ width: '100%', padding: '8px 10px', border: '2px solid #e5e7eb', borderRadius: '8px', fontSize: '13px', outline: 'none', cursor: 'pointer', background: 'white' }}>
                  <option value="all">Todas</option>
                  <option value="none">Sin canastilla</option>
              ...
              {/* Filtro estado */}
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#6b7280', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>🔵 Estado</label>
              ...
              {/* Filtro tipo tubería */}
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#6b7280', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>🔧 Tipo de tubería</label>
                <select value={filterInvType} onChange={e => setFilterInvType(e.target.value)}
                  style={{ width: '100%', padding: '8px 10px', border: '2px solid #e5e7eb', borderRadius: '8px', fontSize: '13px', outline: 'none', cursor: 'pointer', background: 'white' }}>
                  <option value="all">Todos</option>
              ...
              {(filterInvLocation !== 'all' || filterInvBasket !== 'all' || filterInvStatus !== 'all' || filterInvType !== 'all' || searchTerm) && (
                <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                  <button
                    onClick={() => { setFilterInvLocation('all'); setFilterInvBasket('all'); setFilterInvStatus('all'); setFilterInvType('all'); setSearchTerm(''); }}
                    style={{ width: '100%', padding: '8px 10px', background: '#fee2e2', color: '#dc2626', border: '2px solid #fca5a5', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
                    ✕ Limpiar filtros
                  </button>
                </div>
              )}
